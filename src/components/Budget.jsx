...
export default function Budget() {
  ...
  const fetchCurrentBudget = async () => {
    try {
      setIsLoading(true)
      setError(null)
      const response = await fetch(`http://localhost:8080/api/budgets/current?memberId=${memberId}`)
  ...
  const createBudget = async (amount) => {
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/budgets?memberId=${memberId}&amount=${amount}`, {
        method: "POST",
        headers: {
  ...
  const updateBudget = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/budgets/${budget.budgetId}?memberId=${memberId}&newAmount=${newAmount}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/x-www-form-urlencoded",
  ...
  const deleteBudget = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/budgets/${budget.budgetId}?memberId=${memberId}`, {
        method: "DELETE",
      })
